...
export default function EditDetails() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      employment_history: formValues?.employment_history?.map((item) => ({
        ...item,
        start_date: item.start_date ? new Date(item.start_date) : undefined,
        end_date: item.end_date ? new Date(item.end_date) : undefined,
      })),
      educations: formValues?.educations?.map((item) => ({
        ...item,
        degree_completion_date: item.degree_completion_date
          ? new Date(item.degree_completion_date)
          : undefined,
      })),
      achievements: formValues?.achievements?.map((item) => ({
        ...item,
      ...
      publications: formValues?.publications?.map((item) => ({
        ...item,
        date: item.date ? new Date(item.date) : undefined,
      })),
    },
